...
.glass {
	...
	/* OPACITY: 0.1-0.3 for dark bg, 0.5-0.8 fo t bg */

  /* Blur: highe
	border: 1px solid rgba(255, 255, 255, OPACITY);
	  /* AMOUNT: 8-12px subtle, 16-2 rong */

  /* Border: defines edges
	border-radius: YOUR_RADIUS; 255, 255, OPACITY);
...
.neo-raised {
	...
	box-shadow: OFFSET OFFSET BLUR rgba(light-color), -OFFSET -OFFSET BLUR rgba(dark-color);
...
.neo-pressed {
	...
	box-shadow: inset OFFSET OFFSET BLUR rgba(dark-color), inset -OFFSET -OFFSET BLUR rgba(light-color);
}
```
